<div class="col-12 bg-white margin-bottom-15 border-radius-3">
    ...
    <!--suppress HtmlFormInputWithoutLabel -->
        ...
        <button type="submit" class="btn btn-webamooz_net mt-1">اضافه کردن</button>
    </form>
                ...
                <tr role="row" class="">
                    ...
                    <td>
                        ...
                        @can(\Mshm\RolePermissions\Models\Permission::PERMISSION_MANAGE_COURSES)
                            ...
                            @if ($season->status == \Mshm\Course\Models\Season::STATUS_OPENED)
                                <a href="" onclick="updateConfirmationStatus(event,
                                    '{{ route('seasons.lock',$season->id) }}','آیا از قفل کردن این آیتم اطمینان دارید'
                                    ,'قفل شده','status')" class="item-lock mlg-15 text-error" title="قفل کردن"></a>
                            @else
                                <a href="" onclick="updateConfirmationStatus(event,
                                    '{{ route('seasons.unlock',$season->id) }}','آیا از باز کردن این آیتم اطمینان دارید'
                                    ,'باز شده','status')" class="item-lock mlg-15 text-success" title="باز کردن"></a>
                            @endif
                        @endcan
