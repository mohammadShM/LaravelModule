...
@section('content')
    <div class="tab__box">
        <div class="teb__items">
            <a class="tab__item is-active" href="{{ route('courses.create') }}">لیست دوره ها</a>
            <a class="tab__item" href="{{ route('courses.create') }}">دوره های تایید شده</a>
            <a class="tab__item" href="{{ route('courses.create') }}">دوره های تایید نشده</a>
                            ...
                            <tr role="row" class="">
                                ...
                                <td>
                                    @can(\Mshm\RolePermissions\Models\Permission::PERMISSION_MANAGE_COURSES)
                                        ...
                                        <a href="" onclick="updateConfirmationStatus
                                            (event,'{{ route('courses.accept',$course->id) }}'
                                            ,'آیا از تایید این آیتم اطمینان دارید','تایید شده')"
                                           class="item-confirm mlg-15" title="تایید"></a>
                                        <a href="" onclick="updateConfirmationStatus
                                            (event,'{{ route('courses.reject',$course->id) }}'
                                            ,'آیا از رد این آیتم اطمینان دارید','رد شده')"
                                           class="item-reject mlg-15" title="رد"></a>
                                        <a href="" onclick="updateConfirmationStatus
                                            (event,'{{ route('courses.lock',$course->id) }}'
