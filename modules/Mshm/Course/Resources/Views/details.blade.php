                ...
                @can(\Mshm\RolePermissions\Models\Permission::PERMISSION_MANAGE_COURSES)
                    <div class="d-flex item-center flex-wrap margin-bottom-15 operations__btns">
                        <button class="btn all-confirm-btn"
                                onclick="acceptAllLessons('{{ route('lessons.acceptAll',$course->id) }}')">
                            تایید همه جلسات
                        </button>
                        <button class="btn confirm-btn"
                                onclick="acceptMultiple('{{ route('lessons.acceptMultiple',$course->id) }}')">
                            تایید جلسات
                        </button>
                        <button class="btn reject-btn"
                                onclick="rejectMultiple('{{ route('lessons.rejectMultiple',$course->id) }}')">
                            رد جلسات
                        </button>
                        <button class="btn delete-btn"
                                onclick="deleteMultiple('{{ route('lessons.destroyMultiple' , $course->id) }}')">
                            حذف جلسات
                        </button>
                    </div>
                @endcan
                            ...
                            <tr role="row" class="" data-row-id="{{ $lesson->id }}">
                                ...
                                <td class="confirmation_status">
                                       <span class="{{ $lesson->getConfirmationStatusCssClass() }}">
                                           @lang($lesson->confirmation_status)</span>
                                </td>
                                <td class="status">
                                    @if ($lesson->status == \Mshm\Course\Models\Lesson::STATUS_OPENED)
                                ...
                                <td>
                                    @can(\Mshm\RolePermissions\Models\Permission::PERMISSION_MANAGE_COURSES)
                                        <a href="" class="item-delete-me mlg-15" title="حذف"
                                           onclick="deleteItem(event ,'{{ route('lessons.destroy',
                                       [$course->id ,$lesson->id]) }}')"></a>
                                        <a href=""
                                        ...
                                        @if ($lesson->status == \Mshm\Course\Models\Lesson::STATUS_OPENED)
                                            <a href="" onclick="updateConfirmationStatus(event,
                                                '{{ route('lessons.lock',$lesson->id) }}',
                                                'آیا از قفل کردن این آیتم اطمینان دارید'
                                                ,'قفل شده','status')" class="item-lock mlg-15 text-error"
                                               title="قفل کردن"></a>
                                        @else
                                            <a href="" onclick="updateConfirmationStatus(event,
                                                '{{ route('lessons.unlock',$lesson->id) }}',
                                                'آیا از باز کردن این آیتم اطمینان دارید'
                                                ,'باز شده','status')" class="item-lock mlg-15 text-success"
                                               title="باز کردن"></a>
                                        @endif
                                    @endcan
                ...
                <div class="col-12 bg-white margin-bottom-15 border-radius-3">
                    ...
                    <form action="" method="post" class="padding-30">
                        <select name="" id="">
                            ...
                            <option value="1">mohammad1@gmail.com</option>
                            <option value="2">mohammad2@gamil.com</option>
                        </select>
                        <input type="text" placeholder="مبلغ دوره" class="text">
                        <p class="box__title mt-2">کارمزد مدرس ثبت شود ؟</p>
                    ...
                    <div class="table__box padding-30">
                        <table class="table">
                            ...
                            <tr role="row" class="">
                                ...
                                <td><a href="">mohammadshm@gmail.com</a></td>
